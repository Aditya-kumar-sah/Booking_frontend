 ...
 async function addPhotoByLink(ev){
  ...
  return (
    ...
    <div className='flex gap-2'>
         <input className='px-3 py-2 w-full border-2 border-gray-400 my-1 rounded-2xl' value={photolink} onChange={ev => setPhotolink(ev.target.value)}  type='text ' placeholder={'Add using a link ...jpg'}/>
         <button onClick={addPhotoByLink} className='bg-gray-200 px-4 border-2 border-gray-400 rounded-2xl'>Add photo</button>
   </div>
   <div className='mt-2 grid gap-2 grid-cols-3 md:grid-cols-4 lg:grid-cols-6'>
         {(addedphotos.length > 0) && addedphotos.map((link) =>(
               <div className='h-32 flex relative' key={link}>
                   <img className='rounded-2xl w-full object-cover position-center' src = {'https://booking-app-backend-3rkn.onrender.com/uploads/'+link} />
